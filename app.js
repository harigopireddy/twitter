...
//GETTING THE FOLLOWING PEOPLE ID'S

const getFollowingPeopleIdsOfUser = async (username) => {
  const getThFollowingPeopleQuery = `SELECT following_user_id FROM follower
    INNER JOIN user ON user.user_id = follower.follower_user_id
    WHERE user.username = '${username}';`;

  const followingPeople = await db.all(getThFollowingPeopleQuery);
